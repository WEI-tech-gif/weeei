...
def resolve_split_dir(data_yaml_path: str, split_key: str, run_dir: Path | None = None) -> Path | None:
    """
    尽量把 data.yaml 里的 train/val/test 相对路径解析成存在的绝对目录。
    尝试顺序：
    1) 原样（若绝对路径）
    2) 相对 data.yaml 所在目录
    3) 相对当前工作目录 cwd
    4) 相对脚本所在目录（__file__）
    5) 相对 run_dir（如 runs_agropest12/baseline_yolo11n）
    6) 在 data.yaml 父目录向上两层内递归搜 '*/<split_key>/images'
    命中一个即返回；都失败则返回 None。
    """
    ...
    # 6) 递归在 data.yaml 父目录向上两层搜 '*/<split_key>/images'
...
def main():
    ...
    # -------- success / failure 示例图（稳健解析 test 路径）--------
    pred_root = ensure_dir(run_dir / "pred_test")
    success_dir = ensure_dir(run_dir / "success")
    failure_dir = ensure_dir(run_dir / "failure")

    test_dir = resolve_split_dir(DATA, "test")
    if test_dir is None:
        print(f"[WARN] data.yaml Failed to parse to a valid test directory. Skip the success/failure example diagram.")
    else:
        # 这里用 predict(source=目录)；如果 test_dir 仍然为空则不执行
        preds = model.predict(
            source=str(test_dir),
            imgsz=IMG_SIZE,
            conf=CONF_THRES,
        ...
        for r in preds:
            saved = Path(r.save_dir) / Path(r.path).name  # 带框图片
            has_det = (getattr(r, "boxes", None) is not None) and (r.boxes is not None) and (r.boxes.shape[0] > 0)
            dst = success_dir if has_det else failure_dir
            if saved.exists():
                shutil.copy(saved, dst / saved.name)
