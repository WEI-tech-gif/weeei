...
IMAGE_PATH = "valid/images/ants-15-_jpg.rf.3ea6fe71833ba704537533689e7c5d20.jpg"
IMG_SIZE = 640
...
# ================== 手写 Raw Pred 模型：复刻 Detect decode，保持可微 ==================
    ...
    def forward(self, x):
        ...
        for i, m in enumerate(self.model):
            if i == self.detect_idx:
                ...
                # 1) 对每个尺度做 cv2/cv3，得到 head 输出 p[i]
                p = []
                for j in range(nl):
                    feat = feats[j]
                ...
                # 3) 拆 bbox logits 和 cls logits
                box_logits = x_cat[:, : reg_max * 4, :]  # [B,4*reg_max,K]
                cls_logits = x_cat[:, reg_max * 4 :, :]  # [B,nc,K]

                # 4) 用官方 make_anchors 得到 anchor_points & strides（注意：不再乱 transpose）
                anchors, stride_tensor = make_anchors(p, det.stride, 0.5)
                # anchors: [K,2], stride_tensor: [K,1]
                anchors = anchors.to(x_cat.device)
                stride_tensor = stride_tensor.to(x_cat.device)  # [K,1]

                # 5) DFL + dist2bbox -> xywh（特征图坐标），再乘 stride -> 像素坐标
                box_dist = det.dfl(box_logits)  # [B,4,K]
                box_xywh = dist2bbox(
                    box_dist,
                    anchors.unsqueeze(0),  # [1,K,2]
                    xywh=True,
                    dim=1,
                )  # [B,4,K] (单位: cell)
                box_xywh = box_xywh * stride_tensor.view(1, 1, -1)  # [B,4,K]
            ...
            # 非 Detect 层：按 ultralytics 的 m.f 逻辑取输入
            f = getattr(m, "f", -1)
            if isinstance(f, int):
                if f == -1:
                    x_in = x
...
# ================== IoU 加权 Target：让热力图贴着 best_box ==================
class YoloIoUBoxTarget:
    """
    针对 YoloRawPredModel 输出的 [1,K,4+nc]:
      - 计算所有预测框与 best_box 的 IoU
      - IoU > 阈值 的点参与 Ants 类分数加权求和，作为 Grad-CAM 的 loss
      - 若没有满足阈值的点，则退化为该类分数的最大值
    """
    ...
    def __call__(self, model_output):
        ...
        weights = torch.relu(iou - self.iou_thr)  # IoU 小于阈值置 0

        if (weights > 0).any():
            # IoU 加权和：强调与 best_box 对齐、预测为 Ants 的区域
            loss = (cls_scores * weights).sum()
        else:
            # 兜底：没有高 IoU，就用该类分数的最大值
            loss = cls_scores.max()
...
def main():
    ...
    targets = [YoloIoUBoxTarget(best_box_640, ants_cls, IOU_THRESHOLD)]
