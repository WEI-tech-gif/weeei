...
IOU_THRESHOLD = 0.3      # 选 best_k 时用的 IoU 阈值
SHRINK = 0.6             # ROI 收缩比例 (0.4~0.7 可调)
LAMBDA_LOC = 0.6         # 分类 logit vs IoU 定位的权重 (0.3~0.8 可调)
...
def main():
    ...
    # 3) target_layer：Detect 最小 stride 那路
    ...
    sx1 = int(max(0, min(W - 1, (cx - bw / 2) / hi_stride)))
    sx2 = int(max(0, min(W - 1, (cx + bw / 2) / hi_stride)))
    sy1 = int(max(0, min(H - 1, (cy - bh / 2) / hi_stride)))
    sy2 = int(max(0, min(H - 1, (cy + bh / 2) / hi_stride)))
    if sx2 <= sx1:
        sx2 = min(W - 1, sx1 + 1)
    if sy2 <= sy1:
        sy2 = min(H - 1, sy1 + 1)

    # 10) ROI 内逐点 Grad-CAM（关键：不再先平均成 weights）
    grad_pos = torch.relu(dA)

    roi_mask_3d = torch.zeros_like(grad_pos)
    roi_mask_3d[:, sy1:sy2+1, sx1:sx2+1] = 1.0

    grad_pos = grad_pos * roi_mask_3d        # 框外梯度清零
    cam = (grad_pos * A).sum(dim=0)          # [H,W]
    cam = torch.relu(cam)
    ...
    out_path = "yolo11_gradcam_ants_box_roi_elementwise.jpg"
