...
def main():
    ...
    # 收缩 ROI 到框中心 (防止平均被边缘/背景稀释)
    shrink = 0.5  # 0.5~0.8 试
    cx = (bx1 + bx2) * 0.5; cy = (by1 + by2) * 0.5
    bw = (bx2 - bx1) * shrink; bh = (by2 - by1) * shrink
    ...
    if sx2 <= sx1: sx2 = min(W-1, sx1+1)
    if sy2 <= sy1: sy2 = min(H-1, sy1+1)
    roi_grad = dA[:, sy1:sy2+1, sx1:sx2+1]
    ...
    cx_f = max(0, min(W-1, cx_f)); cy_f = max(0, min(H-1, cy_f))
    w_center = torch.relu(dA[:, cy_f, cx_f])    # [C]

    alpha_center = 0.3                           # 0.2~0.5 试
    ...
    cam = torch.relu(cam)
        # -------- 只在 ROI 内保留响应（关键补丁） --------
    roi_mask = torch.zeros_like(cam)
    roi_mask[sy1:sy2+1, sx1:sx2+1] = 1.0

    cam = cam * roi_mask  # 框外直接压成 0
