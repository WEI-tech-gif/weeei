    ...
    def forward(self, x):
        ...
        for i, m in enumerate(self.model):
            if i == self.detect_idx:
                ...
                # 4) 用 make_anchors 生成 anchors/strides
                # 注意：这里恢复成与 Ultralytics Detect 一致的形状处理方式
                ...
                # 5) DFL + dist2bbox -> xywh (像素坐标)
                box_dist = det.dfl(box_logits)              # [B,4,K]
                box_xywh = dist2bbox(
                    box_dist,
                    anchors.unsqueeze(0),  # [1,2,K] 与 [B,4,K] 在 dim=1 对齐
                    xywh=True,
                    dim=1,
                )  # [B,4,K]
                box_xywh = box_xywh * strides  # 广播到 [B,4,K]
