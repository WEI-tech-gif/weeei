...
SHRINK = 0.6             # ROI 收缩比例 (0.4~0.8 自行调)
LAMBDA_LOC = 0.6         # 分类 vs 定位权重 (0.3~0.8 自行调)
...
def get_best_ants_box_640(yolo_model, img_path, img_size, target_class_name):
    """官方 YOLO 推理一次，选出 Ants 类置信度最高的框，并映射到 640x640。"""
    with torch.no_grad():
        ...
        with torch.enable_grad():
            for i, m in enumerate(self.model):
                if i == self.detect_idx:
                    ...
                    x_cat = torch.cat(
                        [pi.view(bs, no, -1) for pi in p], dim=2
                    )  # [B,no,K]
...
def main():
    ...
    # 7) 从 raw preds 中选 best_k（兼顾 IoU 和 logit）
    ...
    # 10) 在特征图上构造缩小后的 ROI（只用来算权重 & mask）
    ...
    roi_mask_2d = torch.zeros((H, W), device=A.device)
    roi_mask_2d[sy1:sy2+1, sx1:sx2+1] = 1.0
    roi_mask_3d = roi_mask_2d.unsqueeze(0)          # [1,H,W]

    # 11) ROI 限定的 Grad-CAM：在 ROI 内平均正梯度得到通道权重
    grad_pos = torch.relu(dA) * roi_mask_3d         # 仅框内算梯度
    weights = grad_pos.mean(dim=(1, 2))             # [C]

    # 标准 Grad-CAM：sum_c w_c * A_c
    ...
    out_path = "yolo11_gradcam_ants_box_roi_gradcam.jpg"
