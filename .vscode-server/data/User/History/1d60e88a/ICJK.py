...
IOU_THRESHOLD = 0.3  # 用于选 best_match anchor（不是区域加权）
...
def get_best_ants_box_640(yolo_model, img_path, img_size, target_class_name):
    ...
    # 映射到 640x640（即使本来就是 640，也这样写更安全）
...
# ================== 手写 Raw Pred 模型（跟你现在跑通的版本一致） ==================
class YoloRawPredModel(nn.Module):
    """
    使用 DetectionModel 的骨干 + Detect 子模块（cv2/cv3/dfl）手写 decode：
      - 不调用 Detect.forward/_inference
      - 输出 [B, K, 4+nc]，前4维为 xyxy，其后为各类 logit（这里保留 logits，方便 Grad-CAM）
    """
        ...
        with torch.enable_grad():
            for i, m in enumerate(self.model):
                if i == self.detect_idx:
                    ...
                    anchors = anchors.transpose(0, 1).contiguous().view(2, -1)   # [2,K]
                    strides = strides.transpose(0, 1).contiguous().view(1, -1)   # [1,K]
...
def main():
    ...
    # 5) 注册 hook：拿到 activations & gradients
    activations = {}
    gradients = {}
    ...
    # 7) 在 raw preds 中找与 best_box 最匹配的那个 anchor (IoU * logit)
    best_box = torch.tensor(best_box_640, dtype=torch.float32, device=device)
    boxes = preds0[:, :4]
    cls_logits = preds0[:, 4 + ants_cls]

    x1 = torch.maximum(boxes[:, 0], best_box[0])
    y1 = torch.maximum(boxes[:, 1], best_box[1])
    x2 = torch.minimum(boxes[:, 2], best_box[2])
    y2 = torch.minimum(boxes[:, 3], best_box[3])
    inter = (x2 - x1).clamp(min=0) * (y2 - y1).clamp(min=0)
    area1 = (boxes[:, 2] - boxes[:, 0]).clamp(min=0) * (boxes[:, 3] - boxes[:, 1]).clamp(min=0)
    area2 = (best_box[2] - best_box[0]).clamp(min=0) * (best_box[3] - best_box[1]).clamp(min=0)
    iou = inter / (area1 + area2 - inter + 1e-6)

    score = cls_logits * torch.clamp(iou, min=0)
    if IOU_THRESHOLD > 0:
        score = torch.where(iou >= IOU_THRESHOLD, score, torch.full_like(score, -1e9))

    best_idx = int(score.argmax().item())
    if score.max() < -1e8:  # 全被阈值淘汰时兜底
        best_idx = int(cls_logits.argmax().item())

    print(f"Best match anchor index={best_idx}, "
          f"cls_logit={float(cls_logits[best_idx]):.4f}, "
          f"IoU={float(iou[best_idx]):.4f}")

    # 8) 以该 anchor 的 Ants logit 为 loss，反向传播
    loss = cls_logits[best_idx]
    loss.backward(retain_graph=True)

    # 9) 取出激活和梯度，做「只在 best_box 内平均」的 Grad-CAM
    h1.remove()
    h2.remove()

    assert "value" in activations and "value" in gradients, "Hooks did not capture activations/gradients."
    ...
    # ★ 关键：只在 best_box 对应的 feature patch 内平均梯度
    patch_grad = grad[:, fy1:fy2 + 1, fx1:fx2 + 1]  # [C, h', w']
    weights = patch_grad.mean(dim=(1, 2))           # [C]

    # Grad-CAM: sum_k (w_k * A_k)
    cam = (weights.view(-1, 1, 1) * act).sum(dim=0)  # [H,W]
    cam = torch.relu(cam)

    cam = cam.detach().cpu().numpy()
    cam = scale_cam_image(cam)  # 归一化到 [0,1]
    cam = cv2.resize(cam, (IMG_SIZE, IMG_SIZE))

    # 10) 叠加到原图，并画框
    cam_image = show_cam_on_image(img_float, cam, use_rgb=True)
    x1_i, y1_i, x2_i, y2_i = map(int, best_box_640)
    cv2.rectangle(cam_image, (x1_i, y1_i), (x2_i, y2_i), (0, 255, 0), 2)

    out_path = "yolo11_gradcam_ants_box_local.jpg"
